...
$( document ).ready(function() {
    ...
    $('body').on('click','.share-fb', function(){//.avata-part
        ...
        var hostname = DOMAIN_SITE.split('//')[1];
        var check = host.includes(hostname);
        ...
        if(check == true){
            ...
            window.open('https://www.facebook.com/sharer/sharer.php?u='+$(this).data('ref'),'facebook-share-dialog','width=500,height=500');
        }
    });
    ...
    $('body').on('click','.share-twitter', function(){//.avata-part
        var type = $(this).data('type');
        var id = $(this).data('id');
        var tag = $(this).data('tag');
        var api_get_linkshr = '';
    ...
    // Share.prototype.network_twitter;
