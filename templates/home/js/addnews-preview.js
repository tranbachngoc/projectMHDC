...
$(document).on('click', '.blockdangtin-dangtinby .post-by-list.show .dropdown-item', function (event) {
    if(!$(this).hasClass('active')){
        $('.blockdangtin-dangtinby .post-by-list.show .dropdown-item').removeClass('active');
        $(this).addClass('active');
        formData.set('sho_id', $(this).data('post-by'));
    }
});
...
function showSlider (effect = 'turn,shift,louvers,cube_over,tv,lines,bubbles,dribbles,glass_parallax,parallax,brick,collage,seven,kenburns,cube,blur,book,rotate,domino,slices,blast,blinds,basic,basic_linear,fade,fly,flip,page,stack,stack_vertical',element = '#boxwork #boxSliderNews') {
    ...
    if(formData.getAll('list_image_id[]').length > 0 ) {
        ...
        console.log('tintuc/listimageslider templates/home/js/addnews-preview.js:1530');
        /*không hoạt động*/
        $.ajax({
...
$(document).on('click', '.addeffectnews', function(e) {
    ...
    $('#boxwork #ms').prop('selectedIndex',0);
    ...
    html_tab += '<div class="boxaddsliderfunction">';
        ...
        html_tab += '<div class="slider-option-item">';
            ...
            html_tab += '<div class="accordion js-accordion">';
            html_tab += '    <input class="hidden hide_0" id="ms" name="audio_slider" type="text">';
            html_tab += '    <div class="accordion-item textaudioselect">';
            html_tab += '        <div class="accordion-toggle textaudioselect-title top-13">';
            html_tab += '            Âm Nhạc';
            html_tab += '        </div>';
            html_tab += '        <div class="accordion-panel textaudioselect-menu w100pc mb00" style="display: none;">';
            html_tab += '            <div class="dropdown-item textaudioselect-item preview audio-from-azibai">';
            html_tab += '                Nhạc trong kho azibai';
            html_tab += '            </div>';
            html_tab += '            <div class="dropdown-item textaudioselect-item">';
            html_tab += '                Nhạc từ thiết bị';
            html_tab += '                <input class="select-music preview select-audio-from-device" name="myFile" type="file">';
            html_tab += '                    <i aria-hidden="true" class="fa fa-upload"></i>';
            html_tab += '                </input>';
            html_tab += '            </div>';
            html_tab += '            <div class="dropdown-item textaudioselect-item preview audio-from-url">';
            html_tab += '                Phát nhạc theo liên kết';
            html_tab += '            </div>';
            html_tab += '        </div>';
            html_tab += '    </div>';
            html_tab += '</div>';
        ...
        $.ajax({
            ...
            success:function(data){
                if(data){
                    ...
                    html += '</select>';
                    audios_azibai_preview = '<div class="modal-body">' + html + '</div>';
                    $('body').append($.fn.tplModal.getHtml(audios_azibai_preview, {
                        'type' : 'modal-dialog-centered modal-dialog-sm slideInUp-animated',
                        'title' : 'Chọn Nhạc trong kho azibai',
                        'class'	: 'preview addnews-audio-from-azibai',
                        'buttons': [
                            'btnNo',
                            {
                                'btn'  : 'btnYes',
                                'class': 'btn-share btn-save-preview-audio-from-azibai',
                            }
                        ],
                    }));
                }
    ...
    if(!audios_preview_url){
        audios_preview_url = '<div class="modal-body"><input placeholder="url audio mp3" type="text" class="input-audio"></div>';
        $('body').append($.fn.tplModal.getHtml(audios_preview_url, {
            'type'  : 'modal-dialog-centered modal-dialog-sm slideInUp-animated',
            'title' : 'Phát nhạc theo liên kết',
            'class'	: 'preview addnews-audio-from-url',
            'buttons': [
                'btnNo',
                {
                    'btn'  : 'btnYes',
...
$(document).on('click', '.preview.audio-from-azibai', function (event) {
    event.preventDefault();
    $('.preview.addnews-audio-from-azibai').modal('show');
});

$(document).on('click', '.preview.audio-from-url', function (event) {
    event.preventDefault();
    $('.preview.addnews-audio-from-url').modal('show');
});

// Thay đổi audio
$(document).on('click', '.preview.addnews-audio-from-azibai .btn-save-preview-audio-from-azibai', function(e) {
    var audio_url = $(".preview.addnews-audio-from-azibai .select-audio").val();
    var source = $('source#audioSource');
    $('#boxwork #ms').val(audio_url);
    source.attr("src", main_url+'media/musics/'+audio_url);
    audio = document.getElementById("audioslider");
    audio.load();
    audio.play();
    $('.preview.addnews-audio-from-azibai').modal('hide');
    $('.accordion .accordion-panel').hide();
});

$(document).on('click', '.preview.addnews-audio-from-url .btn-save-select-audio-from-url', function(e) {
    var audio_url = $('.preview.addnews-audio-from-url .input-audio').val();
    var parser = document.createElement('a');
    parser.href = audio_url;
    if(!parser || !parser.pathname.match(/\.mp3$/ig)){
        return alert('Chỉ chấp nhận liên kết có đuôi .mp3');
    }
    var source = $('source#audioSource');
    $('#boxwork #ms').val(audio_url);
    source.attr("src", audio_url);
    audio = document.getElementById("audioslider");
    audio.load();
    audio.play();
    $('.preview.addnews-audio-from-url').modal('hide');
    $('.accordion .accordion-panel').hide();
});

$(document).on('change', '.preview.select-audio-from-device', function (event) {
    event.preventDefault();
    if($(this).prop('files').length){
        ...
        $.ajax({
            ...
            success: function (res){
                if(res && res.status === 1){
                    var source = $('source#audioSource');
                    $('#boxwork #ms').val(res.data.original);
                    source.attr("src", DOMAIN_CLOUDSERVER +'tmp/'+ res.data.original);
                    audio = document.getElementById("audioslider");
                    audio.load();
                    audio.play();
                    $('.accordion .accordion-panel').hide();
...
$(document).on('click', '#buttonaddnews, #submitnews', function() {
    ...
    var url = main_url+'tintuc/addnewshome';
