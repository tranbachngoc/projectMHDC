...
$(document).on("change",".buttonAddImage",function(event) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        var not_video_url1 = formData.get('not_video_url1');

        if(file.type == 'video/mp4' && not_video_url1 == null) {
...
function renderFileInfo(file,type) {
    console.log('templates/home/js/addnews-person-images.js:75');
...
$(document).on('click', '.editimagegallary', function() {
    ...
    $.ajax({
        url: website_url+'tintuc/getproducthome',
        processData: false,
        contentType: false,
        type: 'GET',
        dataType : 'json',
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.type == 'success') {
                    ...
                    $('#boxaddimagegallery').find('div[data-id="'+element.attr('data-id')+'"]').remove();
                }else {
                    var error_msg = '';
...
$(document).on('click', '.boxaddimagefunction ul li p.activetab', function () {
    ...
    if(tab_cur == 'boxcropfunction') {
        ...
        console.log('addnews-person-images.js:614');
        var dkrm = new Darkroom('#target', {
          // Size options
          minWidth: 100,
          minHeight: 100,
    ...
    if(tab_cur == 'boximageshow') {
        ...
        if(list_link != undefined) {
            console.log('templates/home/js/addnews-person-images.js:658');
    ...
    if(tab_cur == 'boximageffect') {
        ...
        html_effect +='<div class="boxaddeffectimagecontentinner">';
            ...
            html_effect +='<div class="image-main" style="background-image: url('+data_image.image_url+')"></div>';
    ...
    if(tab_cur == 'boxdesfunction') {
        ...
        if(typeof data_image.icon_list != 'undefined') {
            $.each(data_image.icon_list, function( index, item ) {
                addIconHtml('#tabdescontentfeatured',item.image_id,'addimage',item,index);
            });
    ...
    if(tab_cur == 'boxaddtextimage' && typeof data_image.text_list != 'undefined') {
        ...
        if(typeof data_image.text_list.text_color != 'undefined'){
            ...
            $('.boxaddtextimagecontent').find('.slider-text-item').css({'color': data_image.text_list.text_color});
        }
...
$(document).on('click', '#tabdesadd', function () {
    ...
    var data_image = JSON.parse(formData.get('images['+image_id+']'));
    if(typeof data_image.content != 'undefined') {
        var content = data_image.content;
    }else {
        var content = '';
    }
    var sDesImage = '';
...
$(document).on('click', '.inserticon', function(e) {
    ...
    if(error_msg != ''){
        ...
        switch(type) {
            ...
            addIconHtml(element,data_image.image_id,type,data_image.icon_list[num],num);
            break;
            default:
            var icon_list = {
                'icon'      : icon,
            ...
            addIconHtml(element,'',type,icon_list,num);
