	...
	$.fn.jR3DCarousel = function(options){
		...
		(function setup(){
			...
			function createjR3DCarousel(){
                ...
                _jR3DCarouselDiv.addClass('3d-initialized');
		...
		function _playNextSlide(){
            if(_settings.infinite === false && ((_currentSlideIndex + 1)  >= _noOfSlides || _targetSlideIndex < 0)){
                return false;
            }
		...
		function _playPreviousSlide(){
            if(_settings.infinite === false && ((_currentSlideIndex - 1) < 0)){
                return false;
            }
			_targetSlideIndex = _currentSlideIndex - 1;
			_animations.run(_settings.animation, _targetSlideIndex);
		...
		function _getCurrentSlide(){
			// console.log('_currentSlideIndex % _noOfSlides 3',_currentSlideIndex % _noOfSlides);
		...
		function _maintainResposive(){
			...
			var flag = 0;
			if(_noOfSlides !== _container.find('.'+_settings.slideClass).length){
                _noOfSlides = _container.find('.'+_settings.slideClass).length;
                _baseAngle = 360 / _noOfSlides;
                flag = 1;
            }
			...
			_container.find('.'+_settings.slideClass).each(function(i){
				...
				if(!slide.attr('style')){
                    slide.css({position: 'absolute', left: 0, top:0, width:'100%', height:'100%', backfaceVisibility: 'hidden'})
                        .find('img').css({ width:'100%', height:'100%', objectFit:_settings.slideLayout });
				}
				slide.attr('data-index', i).css({ transform: _transform });
            });
			_perspective = _settings.perspective || _perspective;
			...
			if(flag === 1){
                _playSlideByIndex(_currentSlideIndex);
            }
		}
