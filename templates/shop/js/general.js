...
function CheckInput_Contact()
{
	...
	{
		jAlert("Bạn chưa nhập họ tên!",'Yêu cầu nhập',function(r){
	...
	{
		jAlert("Bạn chưa nhập email!",'Yêu cầu nhập',function(r){
	...
	{
		jAlert("Email bạn nhập không hợp lệ!",'Nhập sai',function(r){
			jQuery('#email_contact').focus();														 
		});
		return false;
	}
	...
	{
		jAlert("Bạn chưa nhập địa chỉ!",'Yêu cầu nhập',function(r){
			jQuery('#address_contact').focus();														 
		});
		return false;
	}
	...
	{	
		jAlert("Bạn chưa nhập điện thoại!",'Yêu cầu nhập',function(r){
			jQuery('#phone_contact').focus();														 
		});
		return false;
	}
	...
	if( $('#captcha').length == 1  && $('.inputcaptcha_form').length > 0)
	{
		...
		{
			jAlert("Bạn nhập mã xác nhận chưa đúng!",'Nhập sai',function(r){
...
function addCart(pro_id){
	showLoading();
	// var strURL = this.getDomainName(window.location.hostname);
	...
	$.ajax({
		...
		success: function(result) {
			if(result.pro_type != 0 && result.error == false){
				location.href = '/checkout/v2/'+result.pro_user+'/'+result.pro_type;			
			}
...
//Add san pham vao Cart, tren trang chi tiet sp cua GH
