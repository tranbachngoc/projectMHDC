...
class GiaoHangTietKiem 
{
    ...
    function __construct() {
        $is_dev = env('GHTK_DEV', false);
        if (!$is_dev) {
            $this->URL_REQUEST = 'https://services.giaohangtietkiem.vn/';
        } else {
            $this->URL_REQUEST = 'https://dev.giaohangtietkiem.vn/';
        }
        $this->TokenAzibai = env('GHTK_TOKEN','8cc20D58F9c595840c14Ed2C8fb0C77C1BBBF014');
        //parent::__construct();
    }
    ...
    {
        $result = District::where('DistrictCode',$district)->select('DistrictName','ProvinceName')->first();
