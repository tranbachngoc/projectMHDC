...
class BounceHandler
{
    ...
    public function getHeadersArray($rawHeader)
    {
        ...
        $rawHeader = substr($rawHeader, 0, 10000);
    ...
    protected function getRules()
    {
        ...
        if (is_file($customRulesFile = dirname(__FILE__).'/rules-custom.php')) {
            self::$_rules = array_merge(self::$_rules, require $customRulesFile);
        }

        self::$_rules[self::DIAGNOSTIC_CODE_RULES] = array_merge(self::$_rules[self::DIAGNOSTIC_CODE_RULES], self::$_rules[self::COMMON_RULES]);
        self::$_rules[self::DSN_MESSAGE_RULES]     = array_merge(self::$_rules[self::DSN_MESSAGE_RULES], self::$_rules[self::COMMON_RULES]);
        self::$_rules[self::BODY_RULES]            = array_merge(self::$_rules[self::BODY_RULES], self::$_rules[self::COMMON_RULES]);
