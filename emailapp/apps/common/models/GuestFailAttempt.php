...
class GuestFailAttempt extends ActiveRecord
{
    ...
    {
        $sql = 'SELECT COUNT(*) as counter FROM `' . $this->tableName() . '` WHERE `ip_address_hash` = :hash AND date_added >= DATE_SUB(NOW(), INTERVAL 10 MINUTE)';
        $row = $this->getDbConnection()->createCommand($sql)->queryRow(true, array(':hash' => $this->ip_address_hash));
        if ($row['counter'] >= 10) {
            return true;
        }
        $sql = 'DELETE FROM `' . $this->tableName() . '` WHERE `ip_address_hash` = :hash AND date_added <= DATE_SUB(NOW(), INTERVAL 20 MINUTE)';
        $this->getDbConnection()->createCommand($sql)->execute(array(':hash' => $this->ip_address_hash));
        return false;
    }
