        ...
        <div class="liet-ke-hinh-anh gallery-product gallery-product-popup">
          ...
          <div class="product-tabs js-product-tabs">
            <ul class="nav nav-tabs css-boder">
                <?php if (!empty($result->pro_gallegy)) {
                    foreach ($result->pro_gallegy as $k_pro_gallegy => $v_pro_gallegy) { ?>
                      
                      <li class="js-tab-li" data-key="<?php echo $k_pro_gallegy ?>">
                        <a data-key="<?php echo $k_pro_gallegy ?>" data-toggle="tab" href="#tab_<?php echo $k_pro_gallegy ?>"><?php echo $v_pro_gallegy['content'] ?></a>
                        <div class="close-product btn-delete-cate" data-key=<?php echo $k_pro_gallegy ?>>
                          <img src="/templates/home/styles/images/svg/close_white.svg" alt="">
                        </div>
                        <span class="edit-cate btn-edit-cate" data-key=<?php echo $k_pro_gallegy ?>>
          ...
          <div class="tab-content product-tabs-content product-tabs-content-js">
            ...
            <?php if (!empty($result->pro_gallegy)) {
                    foreach ($result->pro_gallegy as $k_pro_gallegy => $v_pro_gallegy) { ?>                    
                      <div id="tab_<?php echo $k_pro_gallegy ?>" class="tab-pane">
                        <div class="upload-img">
                          <input accept="image/*" class="input_gallegy_upload upload-img-button" data-key=<?php echo $k_pro_gallegy ?> type="file" name="" multiple>
                          <p class="upload-img-text">Chọn hình</p>
                        </div>
                        <div class="grid grid_<?php echo $k_pro_gallegy ?>">
                            <?php 
                              if (!empty($v_pro_gallegy['list_pro'])) { 
                                foreach ($v_pro_gallegy['list_pro'] as $k_list_pro => $v_list_pro) { 
                            ?>
                            <div class="item">
                              <div class="detail">
                                <a> 
                                  <img src="<?php echo $v_list_pro['link'] ?>" alt="" data-w="<?php echo $v_list_pro['detail_w'] ?>" data-h="<?php echo $v_list_pro['detail_h'] ?>">
                                </a>
                                <div class="text class_textarea">
                                  <div class="gallery-product-action">
                                      <textarea class="caption_gallery" type="text" data-key="<?php echo $k_pro_gallegy ?>" data-key-id="<?php echo $k_list_pro ?>" data-gallery-id="<?php echo $v_list_pro->id ?>" rows="4" cols="50"><?php echo $v_list_pro['caption']; ?></textarea>
                                    </div>
                                  </div>
                                  <div class="close-product close-product-js" data-key="<?php echo $k_pro_gallegy ?>" data-key-id="<?php echo $k_list_pro ?>" data-gallery-id="<?php echo $v_list_pro->id ?>">
                                   <img src="/templates/home/styles/images/svg/close_white.svg" alt="">
                                  </div>

                                  <div class="show_caption" data-key="<?php echo $k_pro_gallegy ?>" data-key-id="<?php echo $k_list_pro ?>" data-gallery-id="<?php echo $v_list_pro->id ?>">
...
<script type="text/javascript">
  ...
  var list_tabs = $('.js-product-tabs .js-tab-li');

  if (list_tabs.length > 0) {
    $.each(list_tabs, function( index, value ) {
        list_masonry[index] = $('.grid_' + index).masonry( masonryOptions );
    });
    $('.js-tab-li').first().find('a[data-toggle="tab"]').click();
  }
  ...
  $('.gallery_add_product').click(function(){
    ...
    list_masonry[0].imagesLoaded().progress( function() {
        list_masonry[0].masonry('layout');
    });
  });
