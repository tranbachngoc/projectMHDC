<?php if($this->uri->segment(3) != 'view-album'){?>
<ul class="danhsach-album style-weblink">
    <?php if($is_owns == true) { ?>
        <li class="add js-open-pop-img"><a href="javascript:void(0)"><img class="icon-add" src="/templates/home/styles/images/svg/add_circle.svg" alt="">
        </br><span>Tạo album</span></a></li>
    <?php } ?>
    <?php foreach ($albums as $key => $album) { ?>
    <li>
        <div class="img">
        <a href="<?=$aliasDomain.'library/images/view-album/'.$album->album_id?>" target="_blank">
...
<div class="dieuhuong">
    <div class="button-back">
        <a href="<?=$shop_url .'library/images'?>"><img class="mr10" src="/templates/home/styles/images/svg/prev_bold.svg" alt=""><?=$album->album_name.' ('.$album_total.')'?></a>
    </div>
...
<div class="grid
    ...
    <?php if (!empty($images)){ ?>
        <?php foreach ($images as $index => $image) {?>
            <?php $this->load->view('shop/media/elements/image-item', ['image' => $image, 'index' => $index]) ?>
        <?php } ?>
    <?php } ?>
</div>

<?php $this->load->view('shop/media/album/image/popup-album-image') ?>
<?php $this->load->view('shop/media/album/image/js-popup-album-image') ?>