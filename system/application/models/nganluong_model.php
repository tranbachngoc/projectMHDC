<?php
class Nganluong_model extends CI_Model{
	...
	function __construct()
	{
		...
		$this->merchant_password = MERCHANT_PASS;
		$this->receiver_email = RECEIVER;
	}

	function set_variable($merchant_id, $merchant_password, $receiver_email)
    {
        $this->merchant_id = $merchant_id;
    ...
    */
    ...
    function create_order_nl($order_user,$order_total,$token="",$payment_method="",$bankcode="",$other_info="",$payment_status=0){
        if($order_user) {
            $data = array(
                'date' =>   $date = time(),
            ...
            $this->db->insert('tbtt_payment_order_nl', $data);
    ...
    function getOrderNLDetail($select = "*", $where = ""){
        $this->db->cache_off();
        ...
        #Query
        $query = $this->db->get("tbtt_payment_order_nl");
        $result = $query->row();
        $query->free_result();
        return $result;

    }
    function update_payment_order_nl($id,$status,$token="",$other_info=""){
        $query = "UPDATE `tbtt_payment_order_nl` SET `payment_status` = '".$status."' ";
        if($token !=""){
            $query .= " ,`token` = '".$token."'";
        }
        if($other_info !=""){
            $query .= " ,`other_info` = '".$other_info."'";
        }
        $query .= " WHERE `id` = ".$id;
    ...
    public function listWalletNLLog($params) {
                $this->db->cache_off();
		$this->db->select('*');
		
		if (!empty($params['user_id'])) {
			$this->db->where('order_user', $params['user_id']);
		}
