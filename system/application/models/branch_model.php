...
class Branch_model extends MY_Model
{
    ...
    function get($select = "*", $where = "")
    {
        ...
        #Query
        $query = $this->db->get("tbtt_product_branch");
        $result = $query->row();
        $query->free_result();
        return $result;
    }

    function getConfig($select = "*", $where = "")
    {
        $this->db->cache_off();
        $this->db->select($select);
        if ($where && $where != "") {
        ...
        #Query
        $query = $this->db->get("tbtt_config_branch");
        $result = $query->row();
        $query->free_result();
        return $result;
    }

    function fetch($select = "*", $where = "", $order = "", $by = "DESC", $start = -1, $limit = 0)
    {
        $this->db->cache_off();
        $this->db->select($select);
        if ($where && $where != "") {
    ...
    function fetch_join($select = "*", $join_1, $table_1, $on_1, $join_2, $table_2, $on_2, $where = "", $order = "", $by = "DESC", $start = -1, $limit = 0, $distinct = false)
    {
        ...
        #Query
        $query = $this->db->get("tbtt_user as u");
        $result = $query->result();
        $query->free_result();
        return $result;
    }

    function fetch_join1($select = "*", $join_1, $table_1, $on_1, $where = "", $order = "", $by = "", $start = -1, $limit = 0, $distinct = false)
    {
        $this->db->cache_off();
        $this->db->select($select); 
    ...
    //Use for get list product waiting approve
    function fetch_join2($select='*', $join_1, $table_1, $on_1, $join_2, $table_2, $on_2, $where = "", $order = "", $by = "DESC", $start = -1, $limit = 0, $distinct = false){
    ...
    //Use for get list product waiting approve
    function fetch_join3($select='*', $join_1, $table_1, $on_1, $join_2, $table_2, $on_2, $where = "", $order = "", $by = "DESC", $start = -1, $limit = 0, $distinct = false){
    ...
    {
        return $this->db->insert("tbtt_product_branch", $data);
    }

    function addConfig($data)
    {
        return $this->db->insert("tbtt_config_branch", $data);
    }
    ...
    function update1($data, $where = "")
    {
        ...
        return $this->db->update("tbtt_product_branch", $data);
    }

    function updateConfig($data, $where = "")
    {
        if ($where && $where != "") {
            $this->db->where($where);
        }
        return $this->db->update("tbtt_config_branch", $data);
    }
