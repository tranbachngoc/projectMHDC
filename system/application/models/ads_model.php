...
class Ads_model extends CI_Model
{
	...
	function get($select = "*", $where = "")
	{
		...
		#Query
		$query = $this->db->get("tbtt_ads");
		$result = $query->row();
		$query->free_result();
		return $result;
	}

	function fetch($select = "*", $where = "", $order = "ads_id", $by = "DESC", $start = -1, $limit = 0)
	{
        $this->db->cache_off();
		$this->db->select($select);
		if($where && $where != "")
	...
	function fetch_join($select = "*", $join_1, $table_1, $on_1, $join_2, $table_2, $on_2, $join_3, $table_3, $on_3, $where = "", $order = "ads_id", $by = "DESC", $start = -1, $limit = 0, $distinct = false)
	{
        $this->db->cache_off();
		$this->db->select($select);
    ...
    function add($data)
	{
		...
		$this->db->set($data);
		return $this->db->insert("tbtt_ads");
	}
	...
	function update($data, $where = "")
	{
		...
		return $this->db->update("tbtt_ads", $data);
	}
	...
	function delete($value, $field = "ads_id", $in = true)
    {
		...
		return $this->db->delete("tbtt_ads");
    }
}