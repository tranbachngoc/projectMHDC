...
class Like_product_model extends CI_Model
{
	...
	function get($select = "*", $where = "")
	{
		...
		#Query
		$query = $this->db->get("tbtt_like_product");
		$result = $query->result();
		$query->free_result();
		return $result;
	}
    ...
    function add($data){
		...
		$this->db->insert("tbtt_like_product", $data);
	...
	function update($data, $where = "")
	{
		...
		return $this->db->update("tbtt_like_product", $data);
	}
	...
	function delete($value, $field = "id", $in = true)
    {
		...
		return $this->db->delete("tbtt_like_product");
    }
    ...
    function fetch_join($select = "*", $where = "", $order = "id", $by = "DESC", $start = -1, $limit = 0)
	{
		...
		$this->db->join('tbtt_user', 'tbtt_user.use_id = tbtt_like_product.user_id', 'LEFT');
	...
	function fetch_join4($select, $join_1, $table_1, $on_1, $join_2, $table_2, $on_2, $join_3, $table_3, $on_3, $join_4, $table_4, $on_4, $wherestore = "", $order = "use_id", $by = "DESC", $start = -1, $limit = 0, $distinct = false, $groupby = '')
    {
        ...
        if($groupby && $groupby != ''){
            ...
            $this->db->group_by('tbtt_like_product.pro_id');
        }
        if ($order && $order != "" && $by && ($by == "DESC" || $by == "ASC")) {
            $this->db->order_by($order, $by);
        }
