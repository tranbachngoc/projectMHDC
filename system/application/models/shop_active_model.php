...
class Shop_active_model extends CI_Model
{
	...
	function get($select = "*", $where = "")
	{
		...
		#Query
		$query = $this->db->get("tbtt_shop_active");
		$result = $query->result();
		$query->free_result();
		return $result;
	}
    ...
    function add($data){
		...
		$this->db->insert("tbtt_shop_active", $data);
	...
	function update($data, $where = "")
	{
		...
		return $this->db->update("tbtt_shop_active", $data);
	}
	...
	function delete($value, $field = "id", $in = true)
    {
		...
		return $this->db->delete("tbtt_shop_active");
    }

    function fetch($select = "*", $where = "", $order = "id", $by = "DESC", $group = null, $start = -1, $limit = 0)
    {
        $this->db->cache_off();
