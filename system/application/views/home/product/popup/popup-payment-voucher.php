<!-- modal -->
<!-- Modal -->
<div class="modal" id="popup-payment-voucher">
  <div class="modal-dialog modal-lg modal-dialog-centered modal-mess ">
    <div class="modal-content">
        <!-- Modal Header -->
        <div class="modal-header">
            <h4 class="modal-title js-modal-title"><?=$item_name?></h4>
            <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <!-- Modal body -->
        <form id="form_pay_branch" method="post">
            <div class="modal-body">
                <input type="hidden" name="id" class="id" value="<?=$item_id?>">
                <input type="hidden" name="name" class="name" value="<?=$item_name?>">
                <input type="hidden" name="user_affiliate_key" class="user_affiliate_key" value="<?=$item_user_affiliate_key?>">
                <input type="hidden" name="user_id" class="user_id" value="<?=$item_user_id?>">
                <input type="hidden" name="price" class="price" value="<?=$item_price?>">
                <input type="hidden" name="type_affiliate" class="type_affiliate" value="<?=$item_type_affiliate?>">
                <input type="hidden" name="discount_type" class="discount_type" value="<?=$item_discount_type?>">
                ...
                <div class="creatBranch-content settingBranch-content">
                    <div class="shop_rule_bank">
                      ...
                      <div class="form-group">
                          <label>Tổng tiền: 
                              <span class="total-pay-branch" data-branch="<?php echo $item_price; ?>">
...
<script type="text/javascript">
    ...
    $("body").on("click",".add-pay-branch", function (event) {
        $('.load-wrapp').show();
        var type_pay = parseInt($("#form_pay_branch input.type_pay[type='radio']:checked").val());
        jQuery.ajax({
            type: "POST",
            url: "<?=base_url() . "home/api_affiliate/buy_voucher_on_azibai"?>",
            dataType: 'json',
            data: $('#form_pay_branch').serialize(),
            beforeSend: function () {
              $("#popup-payment-voucher").modal("hide");
            },
    ...
    $("body").on("click",".js-show-popup-payment-voucher", function (event) {
      json_data = JSON.parse($(this).attr("data-json"));
      console.log(json_data);

      $("#popup-payment-voucher").find(".js-modal-title").text(json_data.item_name);

      $("#popup-payment-voucher").find("input[name='id']").val(json_data.item_id);
      $("#popup-payment-voucher").find("input[name='name']").val(json_data.item_name);
      $("#popup-payment-voucher").find("input[name='user_affiliate_key']").val(json_data.item_user_affiliate_key);
      $("#popup-payment-voucher").find("input[name='user_id']").val(json_data.item_user_id);
      $("#popup-payment-voucher").find("input[name='price']").val(json_data.item_price);
      $("#popup-payment-voucher").find("input[name='type_affiliate']").val(json_data.item_type_affiliate);
      $("#popup-payment-voucher").find("input[name='discount_type']").val(json_data.item_discount_type);

      $("#popup-payment-voucher").find(".total-pay-branch").attr("data-branch",json_data.item_price);
      $("#popup-payment-voucher").find(".total-pay-branch").text(formatMoney(json_data.item_price) + " VNĐ");

      $("#popup-payment-voucher").modal("show");
    })
</script>