...
class Product extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('aff_price_model');
...
//            var_dump($id);
    ...
    */
    ...
    public function getProChoose() {
        $af_key = '';
        $oUser = $this->user_model->get("use_id, parent_id,af_key", "use_id = ". (int) $this->session->userdata('sessionUser'));
        ...
        if (!empty($list)) {
            ...
            foreach ($list_user_product as $key_user_pro => $value) {
    ...
    */
    ...
    public function detail_product($productID)
    {
        ...
        if($product->pro_attach != 'null' && $product->pro_attach != null) {
            ...
            if (!empty($list_pro)) 
            {
                ...
                $product_attached = $this->product_model->fetch($select_hh.'pro_id, pro_name, pro_cost, pro_image, pro_dir','pro_id IN (' . $list_pro . ') AND pro_status = 1');
                // $detailproduct = $this->detail_product_model->get('count(tbtt_detail_product.dp_pro_id) as have_num_tqc', 'dp_pro_id = '. (int) $value[1]);

                // foreach ($product_attached as $key_user_pro => $value) {
                //     $dataHH = $this->dataGetHH($value);
                //     $value->hoahong = $this->checkHH($dataHH);
                //     $value->Shopname = $this->get_InfoShop($value->pro_user, 'sho_name')->sho_name;
                // }
        ...
        if(!empty($product))
        {
            ...
            $product->priceSale =  $product->hoahong['price_aff'] != 0 ? $product->hoahong['price_aff'] : $product->hoahong['priceSaleOff'];
        }
        ...
        $listreports = $this->reports_model->fetch('*', 'rp_type = 2 AND rp_status = 1', 'rp_id', 'asc');
        $data['listreports'] = $listreports;
        $is_report = $this->report_detail_model->get('rpd_category_id', 'rpd_by_user = '.(int)$this->session->userdata('sessionUser').' AND rpd_media_id = '.$productID);
        ...
        $typeUserLogin = $this->checkAandD($afflevel_login, $this->session->userdata('sessionUser'));

        if(count($currentuser) > 0 && $currentuser->af_key != '' && $typeUserLogin['type'] == 1){
    ...
    public function getTQC()
    {
        ...
        $where = '';
        if($receive->tqc == 1) {
            $where = ' AND dp_color = "'.$receive->text.'"';
            ...
            if($receive->tqc == 2) {
                $where = ' AND dp_size = "'.$receive->text.'"';
                ...
                $where = ' AND dp_material = "'.$receive->text.'"';
                if($receive->text_color != ''){
                    $where .= ' AND dp_color = "'.$receive->text_color.'"';
                }
                if($receive->text_size != ''){
        ...
        $tqc_select = $this->detail_product_model->get('*', 'dp_pro_id = '.$receive->pro_id.$where);
        //old .' AND (dp_size = "'.$receive->text.'" OR dp_color = "'.$receive->text.'" OR dp_material = "'.$receive->text.'")'
        ...
        if($receive->tqc == 2) {
            $list_style = $this->detail_product_model->fetch('*', 'dp_pro_id = ' .$receive->pro_id . ' AND dp_size = "'.$tqc_select->dp_size.'" AND dp_color = "'.$tqc_select->dp_color.'"');
            foreach ($list_style as $k => $v) {
                $temp = array();
                if ($v->dp_size != '') {
                    $temp['size'] = $v->dp_size;
        ...
        $data = array(
            'pro_user' => $receive->product->pro_user,
            'apply' => $receive->apply,
            'is_product_affiliate' => $receive->is_product_affiliate,
        );

        $dataHH = $this->dataGetHH($receive->product, $receive->af_id);
        $tqc_select->hoahong = $this->checkHH($dataHH, $tqc_select->dp_cost);
        $tqc_select->Shopname = $this->get_InfoShop($receive->product->pro_user, 'sho_name')->sho_name;

        $array = array('tqc_select'=>$tqc_select, 'arr'=>$arr);
