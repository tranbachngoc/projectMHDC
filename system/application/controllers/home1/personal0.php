...
class Personal extends MY_Controller
{
    ...
    public function profile()
    {
        ...
        $getFollow = $this->friend_model->get('id, accept', ['user_id' => (int)$info_public['use_id'], 'add_friend_by' => (int)$this->session->userdata('sessionUser')]);
        if (!empty($getFollow))
        {
        ...
        foreach ($list_news as $key => $item) {
            ...
            foreach ($array as $k => $value) {
                if (strlen($value[0]) > 10) {
                    ...
                    if ($value[1] > 0) {
                        $product = $this->product_model->get('pro_category, pro_user, is_product_affiliate, pro_id, pro_name, pro_sku, pro_cost, pro_currency, pro_image, pro_dir, pro_saleoff, pro_saleoff_value, pro_type_saleoff, end_date_sale, af_amt, (af_rate) as aff_rate'. DISCOUNT_QUERY, 'pro_id = '. (int) $value[1]);
    ...
    public function library_images($news_id = '', $image_id = '')
    {
        ...
        $data['add_friend'] = 0;
        $data['jsclass_'] = ' js-follow-user-profile';
        $getFollow = $this->friend_model->get('id, accept', ['user_id' => (int)$user['use_id'], 'add_friend_by' => (int)$this->session->userdata('sessionUser')]);
        if (!empty($getFollow))
        {
            if($getFollow[0]->accept == 0){
                $data['IsFriend'] = 0;
    ...
    public function library_links()
    {
        ...
        $data['add_friend'] = 0;
        $data['jsclass_'] = ' js-follow-user-profile';
        $getFollow = $this->friend_model->get('id, accept', ['user_id' => (int)$user['use_id'], 'add_friend_by' => $user_id]);
        if (!empty($getFollow)) {
            if($getFollow[0]->accept == 0){
                $data['IsFriend'] = 0;
                $data['add_friend'] = 1;
...
//        die('debug');
    ...
    function friends($type=''){
        ...
        $data['add_friend'] = 0;
        $data['jsclass_'] = ' js-follow-user-profile';
        $getFollow = $this->friend_model->get('id, accept', ['user_id' => $profile, 'add_friend_by' => $session]);
        if (!empty($getFollow))
        {
            if($getFollow[0]->accept == 0){
                $data['IsFriend'] = 0;
    ...
    */
    public function affiliate() {
        ...
        if( in_array($this->session->userdata('affiliate_level'), [1,2,3])) {
        ...
        $data['data_aff'] = Api_affiliate::get_data_home_dashboard($user_id);
    ...
    */
    public function ajaxEditAffiliate() {
        ...
        $aUParent = $this->user_model->get("parent_id,affiliate_level","use_id = " . $iUserId);
        $oAffUser = $this->user_model->get("parent_id,affiliate_level","use_id = " . $id);

        if($aUParent->affiliate_level  >=  $level && $level != 0) {
            $result['message']  = '<div class="error">Bạn chỉ dc cấu hình cho cấp thấp hơn mình!</div>';
            $result['type']     = 'error';
            echo json_encode($result);
            die();
        }

        if(isset($oAffUser->parent_id) && $oAffUser->parent_id == $iUserId) {
            $this->user_model->update(array('affiliate_level' => $level),'use_id = '.$id);
            $result['message']  = 'Sửa affiliate user thành công!';
            $result['type']     = 'success';
        }else {
            $result['message']  = 'Cộng tác viên này không thuộc bạn!';
            $result['type']     = 'error';
        }
        echo json_encode($result);
        die();
    ...
    */
    ...
    public function getAffilateUser () {
        ...
        */
        ...
        // include api
        $this->load->file(APPPATH.'controllers/home/api_affiliate.php', false);
        if(isset($data['is_parent']['type_affiliate']) && $data['is_parent']['type_affiliate'] == 2) {
            $user_id = $data['is_parent']['id'];
        }
        
        $data['data_aff'] = Api_affiliate::get_list_user_affiliate($user_id, $iAff, $page, $limit, $user_type, $search,$data['is_parent']['type_affiliate']);
        $data['search'] = $search;
    ...
    public function ajaxEditService() {
        ...
        if(!empty($this->input->post('input'))) {
            foreach ($this->input->post('input') as $iKI => $aInput) {
                $aListPrice[$aInput['name']] = $aInput['value'];
            }
        }
    ...
    */
    public function affiliateOrder() {
        ...
        $this->load->file(APPPATH.'controllers/home/api_affiliate.php', false);
        $data['data_aff'] = Api_affiliate::get_data_order($user_id, $type_get, $start_date, $end_date, $page, $limit, $search);
    ...
    */
    public function inviteRequest() {
        ...
        if($make_call['status'] == 1) {
            ...
            if(!empty($make_call['msg'])) {
                foreach ($make_call['msg'] as $iKError => $oError) {
                    $result['message']  = $oError[0];
                    break;
                }
            }
    ...
    */
    ...
    public function affiliatelist_detail($service_id = 0)
    {
        ...
        $info_public = $this->info_public;
        if(strcmp($_REQUEST['af_id'], $info_public['af_key']) != 0 && $_REQUEST['type_aff'] == 1) {
    ...
    public function affiliate_withdrawal_bank_step1()
    {
        // Tài add
        // $data['show_sub_aff'] = true;
        $data['not_show_cover'] = true;
        // end
        $this->set_layout('home/personal/profile-layout');
        $this->load->view('home/personal/affiliate/page/page-affiliate-income-withdrawal-banking', $data);
    }

    public function affiliate_withdrawal_momo_step1()
    {
        // Tài add
        // $data['show_sub_aff'] = true;
        $data['not_show_cover'] = true;
        // end
        $this->set_layout('home/personal/profile-layout');
        $this->load->view('home/personal/affiliate/page/page-affiliate-income-withdrawal-momo', $data);
    }
    ...
    public function affiliate_statistic()
    {
        ...
        $this->load->file(APPPATH.'controllers/home/api_affiliate.php', false);
        $data_statistic = Api_affiliate::get_data_statistic($user_id, $start_date, $end_date, $type_view);
    ...
    */
    ...
    private function _checkAffliateOwner() {
        ...
        if($aParentTemp && !empty($aParentTemp)) {
            if($aParentTemp['type_affiliate'] == 2) {
                $aParent = array(
                    'id'            => $aParentTemp['id'],
                    'name'          => $aParentTemp['name'],
                    'type_affiliate'=> 2
                );
            }
        }
