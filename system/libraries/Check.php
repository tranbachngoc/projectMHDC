...
class Check
{
	function is_logined($user, $group, $type = 'admin')
	{
		...
		{
			case 'admin':
			    if($user && trim($user) != '' && (int)$user > 0 && $group && trim($group) != '' && (int)$group >= 4 && (int)$group != 11 && (int)$group != 14 && (int)$group != 15)
			    {
					return true;
			    }
			    break;
			case 'home':
			    if($user && trim($user) != '' && (int)$user > 0 && $group && trim($group) != '' && (int)$group > 0 && (int)$group < 17)
			    {
					return true;
			    }
			    break;
